...
function pauseFromWait(id) {
  ...
  renderPlay(); showMsg(m.name+' 휴식 처리','info');
}
function resumePlayer(id) {
  ...
  renderPlay(); renderAttend(); showMsg(m.name+' 복귀 완료','info');
}
function replaceFromCourt(courtIdx, playerId) {
  if(!requireAdmin()) return;
  var c = courts[courtIdx]; playerId = Number(playerId);
...
function confirmSwap(courtIdx, leavingId, candidateId) {
  ...
  var newPlayers = c.teamA.concat(c.teamB).filter(function(p){return p.id!==leavingId;}).concat([cand]);
  ...
  closeSwapPopup(); renderPlay(); renderAttend();
  showMsg(leaving.name+' → '+cand.name+' 교체','info');
}
function assignSingleCourt(i) {
  if(!requireAdmin()) return;
  if(waitQueue.length<4) { showMsg('대기자가 4명 필요합니다.', 'warn'); return; }
  assignNext(i); renderPlay(); renderAttend();
...
function finishDay() { if(!gameLog.length){showMsg('기록된 게임이 없습니다.','warn');return;} renderResult(); gotoTab('result'); }
function editGame(idx) {
  var g = gameLog[idx]; if(!g) return;
  var newA = prompt('A팀 점수 (현재: '+g.scoreA+')', g.scoreA); if(newA===null) return;
  var newB = prompt('B팀 점수 (현재: '+g.scoreB+')', g.scoreB); if(newB===null) return;
  ...
  g.scoreA=newA; g.scoreB=newB; g.deltaA=ch.deltaA; g.deltaB=ch.deltaB; g.expectedA=ch.expectedA; g.margin=ch.margin;
  ...
  currentSessionSaved=false; renderResult();
...
async function addMember() {
  ...
  byId('n-name').value=''; renderAll();
  setTimeout(function(){
    var row=document.querySelector('[data-elo-id="'+newId+'"]');
    if(row){ var mrow=row.closest('.mrow'); if(mrow){ mrow.style.transition='background .3s'; mrow.style.background='var(--accent-light)'; mrow.style.borderRadius='var(--radius-sm)'; setTimeout(function(){ mrow.style.background=''; }, 2000); } }
  }, 50);
  await syncMembers();
...
async function delMember(id) {
  ...
  present.delete(id); renderAll();
  try {
    var res=await db.from('badminton_members').delete().eq('club_id',CLUB_ID).eq('member_id',id);
    if(res.error) throw res.error;
    setSyncBadge('저장됨','ok'); saveLocal();
  } catch(e){
    members=backup; present.add(id); renderAll();
    setSyncBadge('오류','bad');
...
function bind() {
  ...
  byId('status-toggle').addEventListener('click',function(){
    var list = byId('player-status-list'), toggle = byId('status-toggle');
  ...
  byId('bulk-del-btn').addEventListener('click', async function(){
    ...
    ids.forEach(function(id){present.delete(id);}); selectedForDelete.clear(); renderAll();
    try { await db.from('badminton_members').delete().eq('club_id',CLUB_ID).in('member_id',ids); setSyncBadge('저장됨','ok'); }
    catch(e){ console.warn('bulkDel:',e?.message||String(e)); setSyncBadge('오류','bad'); }
    saveLocal();
  });
  byId('bulk-cancel-btn').addEventListener('click', function(){ selectedForDelete.clear(); renderManage(); });
